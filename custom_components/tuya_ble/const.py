...
# ---------------------------------------------------------------------------
# Inlined Tuya cloud login constants.
#
# These were previously imported from `homeassistant.components.tuya.const`,
# but Home Assistant removed/renamed them when the official Tuya integration
# migrated to QR-code (user-code) login. Importing them caused the Tuya BLE
# config flow to fail with "Config flow could not be loaded: 500".
# We inline stable values here so this integration no longer depends on the
# official Tuya integration's private constants.
# ---------------------------------------------------------------------------
...
# Domain of the official Home Assistant Tuya integration. Used to discover an
# existing Tuya cloud config entry the user may already have set up.
